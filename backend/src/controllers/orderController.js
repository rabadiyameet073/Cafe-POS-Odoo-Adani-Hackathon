...
/**
 * Create order from cart with token validation
 * POST /api/orders/create
 * Requirements: 3.3, 3.4
 */
const createOrderFromCart = catchAsync(async (req, res) => {
    const { table_token, special_instructions } = req.body;
    ...
    const formattedItems = cartItems.map(item => ({
        ...
        variant_price: item.product_variants?.extra_price || 0,
    ...
    logger.info(`Order created from cart: ${result.orderNumber}`);
 ...
 * GET /api/orders/by-token/:tableToken
 * Requirements: 3.4
 */
const getOrdersByToken = catchAsync(async (req, res) => {
    const { tableToken } = req.params;

    const result = await OrderService.getOrdersByTable(tableToken);
