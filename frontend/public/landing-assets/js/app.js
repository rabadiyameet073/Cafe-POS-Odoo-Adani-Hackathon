    ...
    function updateMenu(mode) {
        ...
        items.innerHTML = mode.menuItems.map(function (item, i) {
            ...
            return '<div class="menu-item menu-card glass p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl flex flex-col sm:flex-row items-center gap-4 sm:gap-6 group cursor-pointer relative overflow-hidden" style="border-left:4px solid ' + mode.accentColor + '">' +
                ...
                '<div class="flex items-center justify-end gap-3 sm:gap-4 shrink-0"><span class="font-bold text-xl sm:text-2xl mr-1" style="color:' + mode.accentColor + '">' + item.price + '</span>' +
                '<button type="button" class="menu-add-btn w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center font-bold text-base sm:text-lg transition-colors hover:opacity-90 min-w-[36px] min-h-[36px]" style="background:' + mode.accentColor + ';color:#1a0f0a" aria-label="Add to cart">+</button></div></div>';
        }).join('');
    }
