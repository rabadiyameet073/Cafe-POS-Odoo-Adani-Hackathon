...
const adminNavItems = [
  ...
  { path: 'floors', icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', label: 'Floors' },
  { path: 'tables', icon: 'M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z', label: 'Tables' },
  { path: 'payments', icon: 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z', label: 'Payments' },
  ...
  { path: 'feedback', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z', label: 'Feedback' },
    ...
    <div className="relative min-h-screen overflow-hidden" style={{ background: 'var(--bg-base)', transition: 'background 0.4s ease' }}>
      ...
      <header className={`fixed top-0 left-0 right-0 z-30 premium-topbar${scrolled ? ' scrolled' : ''}`}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          {/* Logo */}
          <Link to="/admin/dashboard" className="flex items-center gap-2.5 group flex-shrink-0">
            <div
            ...
            <div className="flex flex-col leading-none">
              ...
              <span className="text-base font-extrabold tracking-tight transition-colors duration-300" style={{ color: 'var(--text-primary)', lineHeight: 1.15 }}>Admin</span>
            </div>
          </Link>
          ...
          <nav className="hidden md:flex items-center gap-1">
            {adminNavItems.map(item => (
              <Link
                key={item.path}
                to={`/admin/${item.path}`}
                className={`flex items-center gap-2 px-3 py-2 rounded-xl font-medium transition-all nav-tab-item text-sm ${isActive(item.path) ? 'active' : ''}`}
      ...
      >
        {adminNavItems.map(item => (
          <Link
            key={item.path}
            to={`/admin/${item.path}`}
            className="flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl transition-all relative group"
